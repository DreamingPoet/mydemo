...
#include "DesktopPlatformExBase.h"
...
#define LOCTEXT_NAMESPACE "DesktopPlatformEx"


FDesktopPlatformExBase::FDesktopPlatformExBase()
{
	LauncherInstallationTimestamp = FDateTime::MinValue();
}

FString FDesktopPlatformExBase::GetEngineDescription(const FString& Identifier)
{
...
FString FDesktopPlatformExBase::GetCurrentEngineIdentifier()
{
...
void FDesktopPlatformExBase::EnumerateLauncherEngineInstallations(TMap<FString, FString> &OutInstallations)
{
...
void FDesktopPlatformExBase::EnumerateLauncherSampleInstallations(TArray<FString> &OutInstallations)
{
	// Cache the launcher install list if necessary
	ReadLauncherInstallationList();
...
void FDesktopPlatformExBase::EnumerateLauncherSampleProjects(TArray<FString> &OutFileNames)
{
...
bool FDesktopPlatformExBase::GetEngineRootDirFromIdentifier(const FString &Identifier, FString &OutRootDir)
{
...
bool FDesktopPlatformExBase::GetEngineIdentifierFromRootDir(const FString &RootDir, FString &OutIdentifier)
{
	// Get all the installations
	TMap<FString, FString> Installations;
	EnumerateEngineInstallations(Installations);
...
bool FDesktopPlatformExBase::GetDefaultEngineIdentifier(FString &OutId)
{
	TMap<FString, FString> Installations;
	EnumerateEngineInstallations(Installations);
...
bool FDesktopPlatformExBase::GetDefaultEngineRootDir(FString &OutDirName)
{
	FString Identifier;
	return GetDefaultEngineIdentifier(Identifier) && GetEngineRootDirFromIdentifier(Identifier, OutDirName);
}

bool FDesktopPlatformExBase::IsPreferredEngineIdentifier(const FString &Identifier, const FString &OtherIdentifier)
{
...
bool FDesktopPlatformExBase::TryGetEngineVersion(const FString& RootDir, FEngineVersion& OutVersion)
{
...
bool FDesktopPlatformExBase::IsStockEngineRelease(const FString &Identifier)
{
	FGuid Guid;
	return !FGuid::Parse(Identifier, Guid);
}

bool FDesktopPlatformExBase::TryParseStockEngineVersion(const FString& Identifier, FEngineVersion& OutVersion)
{
	TCHAR* End;
...
bool FDesktopPlatformExBase::IsSourceDistribution(const FString &EngineRootDir)
{
...
bool FDesktopPlatformExBase::IsPerforceBuild(const FString &EngineRootDir)
{
	// Check for the existence of a SourceBuild.txt file
...
bool FDesktopPlatformExBase::IsValidRootDirectory(const FString &RootDir)
{
...
bool FDesktopPlatformExBase::SetEngineIdentifierForProject(const FString &ProjectFileName, const FString &InIdentifier)
{
...
bool FDesktopPlatformExBase::GetEngineIdentifierForProject(const FString& ProjectFileName, FString& OutIdentifier)
{
	OutIdentifier.Empty();
...
bool FDesktopPlatformExBase::OpenProject(const FString& ProjectFileName)
{
	FPlatformProcess::LaunchFileInDefaultExternalApplication(*ProjectFileName);
	return true;
}

bool FDesktopPlatformExBase::CleanGameProject(const FString& ProjectDir, FString& OutFailPath, FFeedbackContext* Warn)
{
	// Begin a task
...
bool FDesktopPlatformExBase::CompileGameProject(const FString& RootDir, const FString& ProjectFileName, FFeedbackContext* Warn, ECompilationResult::Type* OutResult)
{
	...
	// Run UBT
	int32 ExitCode;
	bool bResult = static_cast<IDesktopPlatformEx*>(this)->RunUnrealBuildTool(LOCTEXT("CompilingProject", "Compiling project..."), RootDir, Arguments, Warn, ExitCode);
...
bool FDesktopPlatformExBase::GenerateProjectFiles(const FString& RootDir, const FString& ProjectFileName, FFeedbackContext* Warn, FString LogFilePath)
{
...
bool FDesktopPlatformExBase::IsUnrealBuildToolAvailable()
{
...
bool FDesktopPlatformExBase::InvokeUnrealBuildToolSync(const FString& InCmdLineParams, FOutputDevice &Ar, bool bSkipBuildUBT, int32& OutReturnCode, FString& OutProcOutput)
{
...
FProcHandle FDesktopPlatformExBase::InvokeUnrealBuildToolAsync(const FString& InCmdLineParams, FOutputDevice &Ar, void*& OutReadPipe, void*& OutWritePipe, bool bSkipBuildUBT)
{
...
bool FDesktopPlatformExBase::RunUnrealBuildTool(const FText& Description, const FString& RootDir, const FString& Arguments, FFeedbackContext* Warn)
{
	int32 ExitCode;
	return static_cast<IDesktopPlatformEx*>(this)->RunUnrealBuildTool(Description, RootDir, Arguments, Warn, ExitCode);
}
...
const TArray<FTargetInfo>& FDesktopPlatformExBase::GetTargetsForProject(const FString& ProjectFile) const
{
	...
	FString Output;
	int32 ReturnCode = 0;
	if (!FPaths::FileExists(*GetUnrealBuildToolExecutableFilename(FPaths::RootDir())) || !const_cast<FDesktopPlatformExBase*>(this)->InvokeUnrealBuildToolSync(Arguments, *GLog, true, ReturnCode, Output) || ReturnCode != 0)
	{
		...
		if (!GIsBuildMachine)
		{
			const_cast<FDesktopPlatformExBase*>(this)->InvokeUnrealBuildToolSync(Arguments, *GLog, false, ReturnCode, Output);
		}
	}
	...
	{
		UE_LOG(LogDesktopPlatformEx, Warning, TEXT("Unable to read target info for %s"), (ProjectFile.Len() == 0)? TEXT("engine") : *ProjectFile);
	}
...
const TArray<FTargetInfo>& FDesktopPlatformExBase::GetTargetsForCurrentProject() const
{
	return GetTargetsForProject(FPaths::GetProjectFilePath());
}

bool FDesktopPlatformExBase::GetSolutionPath(FString& OutSolutionPath)
{
...
FString FDesktopPlatformExBase::GetDefaultProjectCreationPath()
{
	// My Documents
...
void FDesktopPlatformExBase::ReadLauncherInstallationList()
{
...
void FDesktopPlatformExBase::CheckForLauncherEngineInstallation(const FString &AppId, const FString &Identifier, TMap<FString, FString> &OutInstallations)
{
...
int32 FDesktopPlatformExBase::ParseReleaseVersion(const FString &Version)
{
	TCHAR *End;
...
TSharedPtr<FJsonObject> FDesktopPlatformExBase::LoadProjectFile(const FString &FileName)
{
...
bool FDesktopPlatformExBase::SaveProjectFile(const FString &FileName, TSharedPtr<FJsonObject> Object)
{
	FString FileContents;
...
const FUProjectDictionary &FDesktopPlatformExBase::GetCachedProjectDictionary(const FString& RootDir)
{
	FString NormalizedRootDir = RootDir;
	FPaths::NormalizeDirectoryName(NormalizedRootDir);
...
void FDesktopPlatformExBase::GetProjectBuildProducts(const FString& ProjectDir, TArray<FString> &OutFileNames, TArray<FString> &OutDirectoryNames)
{
...
FString FDesktopPlatformExBase::GetEngineSavedConfigDirectory(const FString& Identifier)
{
...
bool FDesktopPlatformExBase::EnumerateProjectsKnownByEngine(const FString &Identifier, bool bIncludeNativeProjects, TArray<FString> &OutProjectFileNames)
{
	// Get the engine root directory
	FString RootDir;
	if (!GetEngineRootDirFromIdentifier(Identifier, RootDir))
...
bool FDesktopPlatformExBase::BuildUnrealBuildTool(const FString& RootDir, FOutputDevice& Ar)
{
...
bool FDesktopPlatformExBase::ReadTargetInfo(const FString& FileName, TArray<FTargetInfo>& Targets)
{
	// Read the file to a string
	FString Contents;
...
FString FDesktopPlatformExBase::GetUnrealBuildToolProjectFileName(const FString& RootDir) const
{
	return FPaths::ConvertRelativePathToFull(RootDir / TEXT("Engine/Source/Programs/UnrealBuildTool/UnrealBuildTool.csproj"));
}

FString FDesktopPlatformExBase::GetUnrealBuildToolExecutableFilename(const FString& RootDir) const
{
